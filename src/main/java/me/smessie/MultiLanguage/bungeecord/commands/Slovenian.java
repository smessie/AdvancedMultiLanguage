...
public class Slovenian extends Command {
            ...
            if (args.length == 1) {
                ...
                if (Languages.isSupportedLanguage(taal.toUpperCase())) {
                    ...
                    if (Implement.languageEnabled(formatTaal)) {
                        ...
                        player.sendMessage(new TextComponent(ChatColor.GREEN + "Vaš jezik je nastavljen na " + taal + "."));

                        if (Implement.warnOnSelect(taal)) {
                            player.sendMessage(new TextComponent(red + "Pozor! V klepetu ne morete govoriti " + taal + "."));
                        }
                    } else {
                        player.sendMessage(new TextComponent(red + "Ta jezik je onemogočen! :("));
                    }
                    ...
                    player.sendMessage(new TextComponent(red + "Jezik \" + args[0] + \" ni na voljo!"));
                }
            } else {
                player.sendMessage(new TextComponent(red + "Uporaba: /jezik <jezik>"));
            }
        } else {
            sender.sendMessage(new TextComponent(red + "Hé, Samo v igri, lahko igralci nastavijo svoj jezik! :o"));
        }
    }
