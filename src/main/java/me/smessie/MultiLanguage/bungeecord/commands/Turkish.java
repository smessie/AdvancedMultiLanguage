...
public class Turkish extends Command {
            ...
            if (args.length == 1) {
                ...
                if (Languages.isSupportedLanguage(taal.toUpperCase())) {
                    ...
                    if (Implement.languageEnabled(formatTaal)) {
                        ...
                        player.sendMessage(new TextComponent(ChatColor.GREEN + "Dilin " + taal + "olarak değiştirildi."));

                        if (Implement.warnOnSelect(taal)) {
                            player.sendMessage(new TextComponent(red + "Dikkat! Sohbette " + taal + " konuşamazsın."));
                        }
                    } else {
                        player.sendMessage(new TextComponent(red + "Bu dil kullanıma kapalıdır! :("));
                    }
                    ...
                    player.sendMessage(new TextComponent(red + args[0] + " dili bulunamadı!"));
                }
            } else {
                player.sendMessage(new TextComponent(red + "Kullanim: /dil <dil>"));
            }
        } else {
            sender.sendMessage(new TextComponent(red + "Hé, Sadece oyunda olan kişiler kendi dillerini seçebilir! :o"));
        }
    }
