...
public class Slovenian implements CommandExecutor {

    ChatColor red = ChatColor.RED;

    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {

        if (label.equalsIgnoreCase("jezik")) {
                ...
                if (args.length == 1) {
                    ...
                    if (Languages.isSupportedLanguage(taal.toUpperCase())) {
                        ...
                        if (Implement.languageEnabled(formatTaal)) {
                            ...
                            player.sendMessage(ChatColor.GREEN + "Vaš jezik je nastavljen na " + taal + ".");

                            if (Implement.warnOnSelect(taal)) {
                                player.sendMessage(red + "Pozor! V klepetu ne morete govoriti " + taal + ".");
                            }
                        } else {
                            player.sendMessage(red + "Ta jezik je onemogočen! :(");
                        }
                        ...
                        player.sendMessage(red + "Jezik " + args[0] + " ni na voljo!");
                    }
                } else {
                    player.sendMessage(red + "Uporaba: /jezik <jezik>");
                }
            } else {
                sender.sendMessage(red + "Hé, Samo v igri, lahko igralci nastavijo svoj jezik! :o");
            }
        }
        return true;
    }
