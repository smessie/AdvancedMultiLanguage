...
public class Lithuanian implements CommandExecutor {

    ChatColor red = ChatColor.RED;

    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {

        if (label.equalsIgnoreCase("kalba")) {
                ...
                if (args.length == 1) {
                    ...
                    if (Languages.isSupportedLanguage(taal.toUpperCase())) {
                        ...
                        if (Implement.languageEnabled(formatTaal)) {
                            ...
                            player.sendMessage(ChatColor.GREEN + "Tavo kalba yra pakeista į " + taal + ".");

                            if (Implement.warnOnSelect(taal)) {
                                player.sendMessage(red + "Dėmesio! Tu negali susirašinėti " + taal + ".");
                            }
                        } else {
                            player.sendMessage(red + "Ši kalba nėra aktyvuota! :(");
                        }
                        ...
                        player.sendMessage(red + args[0] + " kalba nerasta!");
                    }
                } else {
                    player.sendMessage(red + "Naudojimas: /kalba <kalba>");
                }
            } else {
                sender.sendMessage(red + "Hé, Tik serverio žaidėjai gali nustatyti savo kalbą! :o");
            }
        }
        return true;
    }
