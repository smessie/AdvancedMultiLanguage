...
public class Portuguese implements CommandExecutor {

    ChatColor red = ChatColor.RED;

    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {

        if (label.equalsIgnoreCase("linguagem")) {
                ...
                if (args.length == 1) {
                    ...
                    if (Languages.isSupportedLanguage(taal.toUpperCase())) {
                        ...
                        if (Implement.languageEnabled(formatTaal)) {
                            ...
                            player.sendMessage(ChatColor.GREEN + "Sua linguagem foi setada para " + taal + ".");

                            if (Implement.warnOnSelect(taal)) {
                                player.sendMessage(red + "Atenção! Você não pode falar " + taal + " no chat.");
                            }
                        } else {
                            player.sendMessage(red + "Essa lingua está desativada! :(");
                        }
                        ...
                        player.sendMessage(red + "A lingua " + args[0] + " não foi encontrada!");
                    }
                } else {
                    player.sendMessage(red + "Use: /linguagem <linguagem>");
                }
            } else {
                sender.sendMessage(red + "Hé, Apenas jogadores dentro do jogo podem setar sua linguagem! :o");
            }
        }
        return true;
    }
