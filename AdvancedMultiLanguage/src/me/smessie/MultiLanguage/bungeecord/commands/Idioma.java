...
public class Idioma extends Command {
			...
			if(args.length == 1) {
				...
				if(Languages.languages.contains(taal.toUpperCase())) {
					...
					if(Implement.languageEnabled(formatTaal)) {
						...
						player.sendMessage(new TextComponent(red + "Este idioma se encuentra deshabilitado! :("));
					}
				} else
				...
				if(Languages.languagesOwn.containsKey(taal.toLowerCase())) {
					...
					player.sendMessage(new TextComponent(red + "No se ha encontrado el idioma " + args[0] + "!"));
				}
			} else {
				player.sendMessage(new TextComponent(red + "Uso: /idioma <idioma>"));
			}
		} else {
			sender.sendMessage(new TextComponent(red + "Hé, solo los jugadores pueden modificar su idioma! :o"));
		}
	}
